...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
  ...
  if (empty($_POST["email"])) {
    ...
    if (!filter_var($email, FILTER_VALIDATE_EMAIL)) {
      $emailErr = "<span style='font-family: RobotoMono;color:red;'>Invalid email format skill issue</span>";
    }
  }
  ...
  if (empty($_POST["website"])) {
    ...
    if (!preg_match("/\b(?:(?:https?|ftp):\/\/|www\.)[-a-z0-9+&@#\/%?=~_|!:,.;]*[-a-z0-9+&@#\/%=~_|]/i",$website)) {
      $websiteErr = "<span style='font-family: RobotoMono;color:red;'>Invalid URL skill issue</span>";
    }
  }
